...
** To find the container images needed for pods that are currently running on the base CaaSP cluster (repeat for all nodes in the cluster to see images needed for all running containers):
----
...
for EACH in $(sudo podman images | grep suse  | grep caasp | grep -v REPOSITORY | awk '{print$1":"$2}'); do LOCAL=$(echo $EACH | awk -F/ '{print$4}'); sudo podman tag $EACH ${HOSTNAME}:5000/$LOCAL; sudo podman push ${HOSTNAME}:5000/$LOCAL; done
for EACH in $(sudo podman images | grep nfs-client-provisioner | grep -v REPOSITORY | awk '{print$1":"$2}'); do LOCAL=$(echo $EACH | awk -F/ '{print$3}'); sudo podman tag $EACH ${HOSTNAME}:5000/$LOCAL; sudo podman push ${HOSTNAME}:5000/$LOCAL; done
----
